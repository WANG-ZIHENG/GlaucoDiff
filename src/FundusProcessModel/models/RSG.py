...
class RSG(nn.Module):
    def __init__(self, n_center=3, feature_maps_shape=[32, 16, 16], num_classes=10, contrastive_module_dim=128,
                 head_class_lists=[], transfer_strength=1.0, epoch_thresh=100):
    ...
    def compute_cesc_loss(self, feature_maps, c, gamma, target, epoch):
        ...
        if epoch <= self.epoch_thresh:
            ...
            loss = torch.sum(torch.bmm(gamma,
                                       torch.pow((feature_maps.unsqueeze(1).expand(-1, c.size()[1], -1, -1, -1) - c),
                                                 2).view(num, self.n_center, -1))) / (num) + F.cross_entropy(pair_pred,
                                                                                                             target_cat)
        else:
            loss = torch.sum(torch.bmm(gamma,
                                       torch.pow((feature_maps.unsqueeze(1).expand(-1, c.size()[1], -1, -1, -1) - c),
                                                 2).view(num, self.n_center, -1))) / (num)
        return loss
    ...
    def compute_mv_loss(self, origin_feature, origin_center, target_center, target_features, gamma_head, target,
                        gamma_tail):
        ...
        loss = F.cross_entropy(pair_pred, torch.zeros(num).long().cuda()) + \
               (torch.sum(torch.abs(torch.norm(var_map_t.view(-1, self.C), dim=1) - torch.norm(var_map.view(-1, self.C),
                                                                                               dim=1))) + torch.sum(
                   torch.abs(target_features_norm * var_map_norm - torch.ones(target_features_norm.size()).cuda()))) / (
                   num)
    ...
    def forward(self, feature_maps, head_class_lists, target, epoch):
        ...
        centers_ = self.centers[target_select, :, :].view(-1, self.n_center, maps_detach.size()[1]).unsqueeze(
            3).unsqueeze(4).repeat(1, 1, 1, maps_detach.size()[2], maps_detach.size()[3])
        ...
        if num_tail != 0 and num_head != 0 and epoch > self.epoch_thresh:
            if num_head >= num_tail:
                ...
                for j in range(0, segment):
                    ...
                    center_origin = self.centers[target_head_select, :, :].view(-1, self.n_center,
                                                                                maps_detach.size()[1]).unsqueeze(
                        3).unsqueeze(4).repeat(1, 1, 1, maps_detach.size()[2], maps_detach.size()[3])
                    ...
                    target_center = self.centers[target_tail_select, :, :].view(-1, self.n_center,
                                                                                maps_detach.size()[1]).unsqueeze(
                        3).unsqueeze(4).repeat(1, 1, 1, maps_detach.size()[2], maps_detach.size()[3])
                    loss_mv, feature_f = self.compute_mv_loss(feature_origin, center_origin, target_center, latent_2,
                                                              gamma_head, target_tail, gamma_tail)
